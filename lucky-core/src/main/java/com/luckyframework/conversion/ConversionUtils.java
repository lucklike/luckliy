...
public abstract class ConversionUtils {
     ...
     * @return 转换后的对象
     */
    public static Object conversion(Object toConvertValue, ResolvableType resolvableType, List<ConversionService> conversions, Function<Object,Object> function){
        ...
        if(Collection.class.isAssignableFrom(rawClass)){
            return conversionToCollection(functionValue, resolvableType, conversions, function);
        }

        // 转化的目标类为数组类型
        if(rawClass.isArray()){
            return conversionToArray(functionValue, resolvableType, conversions, function);
        }

        // 转化的目标类为Map类型
        if(Map.class.isAssignableFrom(rawClass)){
            return conversionToMap(functionValue, resolvableType, conversions, function);
        }


        // 泛型toString()相同
        if (resolvableType.toString().equals(ResolvableType.forClass(toConvertValue.getClass()).toString())){
            return functionValue;
        }

        // 转化的目标类为基本类型
        if(ClassUtils.isSimpleBaseType(rawClass)){
            return conversionToBaseType(functionValue, rawClass);
        }
        ...
        return conversionToPojo(functionValue, resolvableType, conversions, function);
    }
    ...
    private static Class<?> conversionToClass(Object toConvertValue) {
        return ClassUtils.forName(toConvertValue.toString(),Thread.currentThread().getContextClassLoader());
    }
     ...
     * @return 目标实体
     */
    private static Object conversionToPojo(Object toConvertValue, ResolvableType resolvableType, List<ConversionService> conversions, Function<Object, Object> function) {
        Class<?> rawClass = resolvableType.getRawClass();
        ...
        // 转化类型为文件类型
        if(File.class.isAssignableFrom(rawClass)){
        ...
        // 转化类型为OutputStream类型
        if(OutputStream.class.isAssignableFrom(rawClass)){
            return conversionToOutputStream(toConvertValue);
        }

        // 转化类型为URL
        if(URL.class.isAssignableFrom(rawClass)){
            return conversionToURL(toConvertValue);
        }

        // 转化类型为URI
        if(URI.class.isAssignableFrom(rawClass)){
        ...
        if(toConvertValue instanceof Map){
            ...
            Field[] fields = ClassUtils.getAllFields(rawClass);
            Object resultPojo = ClassUtils.newObject(rawClass);
            for (Field field : fields) {
                String mappingName = getMappingName(field);
                if(valueMap.containsKey(mappingName)){
                    FieldUtils.setValue(resultPojo, field, conversion(valueMap.get(mappingName), ResolvableType.forField(field,rawClass), conversions, function));
                }
            }
            return resultPojo;
        }else {
            try {
                for (ConversionService conversion : conversions) {
                    if(conversion.canConvert(resolvableType, ResolvableType.forInstance(toConvertValue))){
                        conversion.addUseConversions(conversions);
                        return conversion.conversion(toConvertValue);
                    }
                }
                return conversionToPojo(pojoToMap(null, toConvertValue), resolvableType, conversions, function);
     ...
     * @return 数组对象
     */
    private static Object conversionToArray(Object toConvertValue, ResolvableType resolvableType, List<ConversionService> conversions, Function<Object,Object> function) {

        ResolvableType resultArrayEntryType = resolvableType.getComponentType();
        ...
        if(specialTypeArray != null){
            return ContainerUtils.arrayDowncasting(specialTypeArray, resultArrayEntryClass);
        }
    ...
    private static <T> T conversionToBaseType(Object toConvertValue, Class<T> baseType){
        String stringValue;
        Class<?> toConvertValueClass = toConvertValue.getClass();

        // 数组类型，取第一个元素
        if(toConvertValueClass.isArray()){
            stringValue = Array.get(toConvertValue,0).toString();
        }
        // 集合类型，取第一个元素
        else if(Collection.class.isAssignableFrom(toConvertValueClass)){
            Collection<?> collectionValue = (Collection<?>) toConvertValue;
            List<?> list = new LinkedList<>(collectionValue);
            stringValue = list.get(0).toString();
        }
        // 其他类型
        else{
            stringValue = toConvertValue.toString();
        }

        boolean canCalculated = ClassUtils.isCanCalculated(baseType);

        try {
            return JavaConversion.fromString(toConvertValue.toString(),baseType,canCalculated);
        }catch (Exception e){
