...
public class HttpClientExecutor implements HttpExecutor {
                ...
                for (com.luckyframework.httpclient.core.Header header : headerList) {
                    ...
                    if(headerValue != null){
                        switch (header.getHeaderType()){
                            case ADD: httpRequestBase.addHeader(headerName,headerValue.toString());break;
                            case SET: httpRequestBase.setHeader(headerName,headerValue.toString());break;
                        }
                    }
                }
            }
    ...
    protected void resultProcess(ResponseProcessor processor,CloseableHttpResponse response) throws IOException {
        ...
        processor.process(code,httpHeaderManager,inputStream);
    }
    ...
    private HttpRequestBase createHttpClientRequest(Request request) throws IOException {
        switch (request.getRequestMethod()){
            case GET        : return createHttpGet(request);
            case POST       : return createHttpPost(request);
            case DELETE     : return createHttpDelete(request);
            case PUT        : return createHttpPut(request);
            case HEAD       : return createHttpHead(request);
            case OPTIONS    : return createHttpOptions(request);
            case TRACE      : return createHttpTrace(request);
            case PATCH      : return createHttpPatch(request);
            default         : throw new NotFindRequestException("Apache Http Client does not support requests of type ['"+request.getRequestMethod()+"'].");
        }
    }
    ...
    private HttpEntity getHttpEntity(Request request) throws IOException {
        ...
        return isFileRequest(nameValuesMap)
                ? getFileHttpEntity(nameValuesMap)
                : getUrlEncodedFormEntity(nameValuesMap);
    }
    ...
    private static HttpEntity getFileHttpEntity(Map<String,Object> multipartFileParams) throws FileNotFoundException {
        ...
        for (Map.Entry<String, Object> e : multipartFileParams.entrySet()) {
            ...
            if (File.class == paramValueClass) {
                ...
                builder.addBinaryBody(e.getKey(), new FileInputStream(file), org.apache.http.entity.ContentType.MULTIPART_FORM_DATA, file.getName());//文件参数-File
            }
            //包装File[]类型的参数
            ...
            //包装MultipartFile类型的参数
                ...
                builder.addBinaryBody(e.getKey(), mf.getInputStream(), org.apache.http.entity.ContentType.MULTIPART_FORM_DATA, mf.getFileName());//文件参数-MultipartFile
            }
