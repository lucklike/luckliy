...
| 重要的类                                                                                                               | 说明                                                                                                                                           | 主要方法                                                                                                                                                                                                                                                                                                                                                                                         |
|--------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [FileChunk.java](../../../src/main/java/com/luckyframework/httpclient/generalapi/chunk/FileChunk.java)             | `文件快`，表示一个已经切分好的文件块，包含文件块的`索引信息`、`分片起始位置信息`、`分片内容`信息等                                                                                        | `byte[] getContent()`：获取分片文件内容<br/>`long getIndex()`:获取索引信息<br/>`File getFile()`: 获取文件实例                                                                                                                                                                                                                                                                                                     |
| [FileChunkHandle.java](../../../src/main/java/com/luckyframework/httpclient/generalapi/chunk/FileChunkHandle.java) | 文件块处理器，该对象需要使用`File对象`以及`分片大小`信息来构建，<br/>内部会自动将`File对象`进行`分片`生成一系列`的FileChunk对象`，<br/>其中的`asyncHandle方法`提供了用于处理分片文件的逻辑，可以调用该方法来完成异步分片文件上传的逻辑 | 获取实例对象：<br/>`static FileChunkHandle of(File, long)`<br/>`[0] File `: 原文件 <br/>`[1] long `: 分片大小（单位：byte）<br/> 分片文件处理方法：<br/>`void asyncHandle(Executor, Function<FileChunk, T>, Consumer<HandleResults<T>>) `<br/> `[0] Executor`: 用于异步执行分片文件的线程池<br/> `[1] Function<FileChunk, T>`: 分片文件的处理逻辑，处理完成之后返回一个结果<br/> `[2] Consumer<HandleResults<T>>`: 所有分片文件都处理完成后执行，用于结果校验以及后续操作(例如：通知服务器合并文件) |
| [HandleResults.java](../../../src/main/java/com/luckyframework/httpclient/generalapi/chunk/HandleResults.java)     | 分片文件的处理结果， `asyncHandle方法`执行结束后会为每个FileChunk对象都生成<br/>一个HandleResult对象，该对象可以用来判断分片文件处理是否成功                                                   | `ResultClassify<R> classify(Function<R, Boolean>)`：<br/>传入一个结果判断的逻辑，使用该逻辑对所有结果集分类，最终得到一个分好类的结果对象`ResultClassify`                                                                                                                                                                                                                                                                             |
| [ResultClassify.java](../../../src/main/java/com/luckyframework/httpclient/generalapi/chunk/ResultClassify.java)   | 分好类的结果集，该类中的结果集已经被分为了两类：`成功结果集`、`失败结果集` ，使用该类可以更加方便的来处理结果                                                                                    | ` boolean isAllSuccess()`：是否全部成功 <br/>  `void failureForEach(Consumer<HandleResult<R>>)`: 处理失败结果<br/> ` HandleResults<R> getFailure()` ：获取所有失败的结果集                                                                                                                                                                                                                                           |
