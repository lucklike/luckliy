...
#### 常见的应用场景：
- **浏览器访问网站**：浏览器通过 HTTPS 与服务器建立加密通信，验证服务器身份，防止用户受到伪造网站的欺骗。
- **API 接口通信**：客户端与服务器之间的通信，客户端确保所请求的 API 来自合法的服务器。
这种单向认证模式常见于大多数 HTTPS 连接中，因为它简化了客户端和服务器之间的认证过程。
...
public interface SSLApi {
   ...
   /*
    使用当前对象的keyStore()方法来获取KeyStore信息来验证服务器证书
  */
...
- **增强安全性**：双向认证确保双方的身份都经过了验证，不会发生伪造服务器或客户端的情况。
- **防止中间人攻击**：中间人无法伪装成合法的服务器或客户端，无法篡改通信内容。
- **适用于高安全场景**：常用于金融交易、企业内部的敏感数据传输等场景。

**双向认证的挑战：**

- **证书管理**：客户端需要一个有效的证书，而每个客户端都需要保管自己的私钥。证书的管理、更新和分发可能增加系统的复杂度。
- **性能问题**：双向认证增加了 SSL/TLS 握手的复杂度，需要更多的计算和通信，这可能影响性能，特别是在客户端数量非常大的情况下。
- **配置难度**：配置双向认证需要服务器和客户端都配置 SSL/TLS 证书，且双方必须配置正确的证书链，这对开发者和运维人员的要求较高。